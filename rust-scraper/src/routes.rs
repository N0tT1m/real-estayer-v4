...
use crate::{
    database,
    models::*,
    scraping::{scrape_region, send_email, AmenityFilter},
    watchlist::*,
};
use anyhow::Result;
use axum::{
...
static SCRAPE_STATUS: Lazy<StdMutex<ScrapeStatus>> =
    Lazy::new(|| StdMutex::new(ScrapeStatus::default()));
...
fn get_chrome_version() -> Result<String> {
    let output = if cfg!(target_os = "windows") {
        ...
        Command::new("google-chrome").arg("--version").output().ok()
    };
...
#[axum::debug_handler]
pub async fn scrape_everything(Query(params): Query<HashMap<String, String>>) -> impl IntoResponse {
    ...
    use crate::scraping::{preset_bbox, BoundingBox};
    let preset = params
        .get("preset")
        .cloned()
    ...
    let bbox = match (
        ...
        (Some(ne_lat), Some(ne_lng), Some(sw_lat), Some(sw_lng)) => BoundingBox {
            ne_lat,
            ne_lng,
            sw_lat,
            sw_lng,
    ...
    tokio::spawn(async move {
        ...
        use crate::scraping::{scrape_region_tiled, GuestParams};
...

        {
            ...
            *status = ScrapeStatus {
                ...
                failed: 0,
                message: format!(
                    "Region scrape starting (preset={}, max_depth={}, enrich={})",
                    preset, max_depth, enrich
                ),
        ...
        match result {
            Ok((inserted, tiles)) => {
                log::info!(
                    "[EVERYTHING] Completed: {} listings from {} tiles",
                    inserted,
                    tiles
                );
                let mut status = SCRAPE_STATUS.lock().unwrap();
                ...
                status.message = format!(
                    "Completed! {} listings inserted from {} tiles",
                    inserted, tiles
                );
...
#[axum::debug_handler]
pub async fn scrape_city_data(Query(params): Query<HashMap<String, String>>) -> impl IntoResponse {
    log::info!("[SCRAPE_CITY] ========== NEW SCRAPE REQUEST ==========");
    log::info!(
        "[SCRAPE_CITY] Received scrape request with params: {:?}",
        params
    );
    ...
    let check_in_date = params
        .get("check_in")
        .or_else(|| params.get("check_in_date"))
        .cloned()
        .unwrap_or_default();
    let check_out_date = params
        .get("check_out")
    ...
    tokio::spawn(async move {
        ...
        let result: Result<Vec<Listing>, anyhow::Error> = async {
            use crate::scraping::{scrape_city_fast, GuestParams};
            ...
            let check_in_opt = if !check_in_date.is_empty() {
                Some(check_in_date.as_str())
            } else {
                None
            };
            let check_out_opt = if !check_out_date.is_empty() {
                Some(check_out_date.as_str())
            } else {
                None
            };
            let guest_params = GuestParams::new(adults, children, infants, pets);
            ...
            info!(
                "[FAST] Scraping {} (tiled={}, enrich={})",
                search_location, tiled, enrich
            );
            ...
            for mut details in scraped.into_iter() {
                let has_badge = details.features.iter().any(|f| {
                    ...
                    f.contains("superhost")
                        || f.contains("guest fav")
                        || f.contains("guest-fav")
                        || f.contains("rare find")
                        || f.contains("highly rated")
                        || f.contains("top rated")
                });
                ...
                let passes_amenity = (!hot_tub || has_hot_tub)
                    && (!pool || has_pool)
            ...
            info!(
                "[FAST] {} listings after filtering (from {})",
                kept.len(),
                found
            );
            {
            ...
            info!(
                "[FAST] Successfully inserted {} listings",
                inserted_ids.len()
            );
        ...
        match result {
            Ok(listings) => {
                ...
                // Update status to completed
                {
                    ...
                    status.status = "completed".to_string();
                    status.message =
                        format!("Completed! {} listings saved to database", listings.len());
                    status.completed_at = Some(chrono::Utc::now().to_rfc3339());
                }
            }
            Err(e) => {
    ...
    log::info!(
        "[SCRAPE_CITY] Scrape task spawned for city: {}, returning immediately",
        city_for_response
    );
    Json(json!({
        "status": "started",
...
pub async fn get_listings_without_limit(Path(city): Path<String>) -> impl IntoResponse {
    let result = async {
...
pub async fn get_listing(Path(listing_id): Path<String>) -> impl IntoResponse {
    let result = async {
        let object_id =
            ObjectId::parse_str(&listing_id).map_err(|_| anyhow::anyhow!("Invalid listing ID"))?;
    ...
    match result {
        ...
        Err(e) => {
            let (status, message) = match e.to_string().as_str() {
                ...
                _ => (StatusCode::INTERNAL_SERVER_ERROR, "Failed to get listing"),
            };
...
pub async fn test_stealth_browser() -> impl IntoResponse {
    async fn run_test() -> Result<serde_json::Value> {
        ...
        for i in 1..=10 {
            ...
            log::info!(
                "[TEST-STEALTH] Check {}/10: valid={}, url={}",
                i,
                is_valid,
                current_url
            );
        ...
        let has_listings = source.contains("StaySearchResult")
            || source.contains("listing")
            || source.contains("homes");

        log::info!(
            "[TEST-STEALTH] Test complete! Session lasted 10+ seconds, has_listings={}",
            has_listings
        );
...
pub async fn remove_from_watchlist(
    Path((user_id, listing_id)): Path<(String, String)>,
) -> impl IntoResponse {
    let mut service = WATCHLIST_SERVICE.lock().await;
