...
func TestBookingPartnerGate(t *testing.T) {
	s := NewBookingPartnerService("AID", "")
	if s.Configured() {
		t.Errorf("missing Demand key → not configured")
	}
	_, err := s.Search(context.Background(), StaySearchRequest{Destination: "Lisbon"})
	if !errors.Is(err, ErrBookingNotConfigured) {
		t.Errorf("expected ErrBookingNotConfigured, got %v", err)
	}
	if !NewBookingPartnerService("aid", "key").Configured() {
		t.Errorf("both set → configured")
	}
}

func TestExpediaPartnerGate(t *testing.T) {
	// Expedia needs BOTH key and secret.
	for _, c := range []struct {
		...
		got := NewExpediaPartnerService(c.key, c.secret).Configured()
		if got != c.ok {
			t.Errorf("expedia(%q/%q).Configured = %v; want %v", c.key, c.secret, got, c.ok)
		}
	}

	_, err := NewExpediaPartnerService("", "").Search(context.Background(), StaySearchRequest{Destination: "Lisbon"})
	if !errors.Is(err, ErrExpediaNotConfigured) {
		t.Errorf("expected ErrExpediaNotConfigured, got %v", err)
	}
}
